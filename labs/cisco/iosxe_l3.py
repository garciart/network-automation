    ...
    def run(self, reporter, connection_type=None, **options):
        ...
        try:
            if connection_type == 'serial':
                ...
                child = router.connect_via_telnet(reporter,
            ...
            router.set_router_ip_addr(child, reporter, '', self._ethernet_port,
                                      self._device_ip_addr, self._subnet_mask, commit=False)
            router.ping_from_device(child, reporter, '', self._pma_ip_addr, count=4)
            reporter.note(run_cli_command('ping -c 4 {0}'.format(self._pma_ip_addr)))
            router.download_from_device_tftp(child,
                                             reporter,
                                             self._eol,
                                             self._ethernet_port,
                                             'startup-config',
            ...
            router.upload_to_device_tftp(child,
                                         reporter,
                                         self._eol,
                                         self._pma_ip_addr,
                                         self._config_file_path,
