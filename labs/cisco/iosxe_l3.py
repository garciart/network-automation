# -*- coding: utf-8 -*-
"""Test of the CiscoIOS class (cisco_ios.py) using the Cisco IOS-XE router image in GNS3.
"""
import os.path
import sys
...
__all__ = ['CiscoIOSXEL3']


class CiscoIOSXEL3(object):
    _cisco_prompts = [
    ...
    def run(self, reporter, connection_type='serial', **options):
        router = CiscoIOS(self._device_hostname)
        child = None
        try:
            # Connect to the device
            if connection_type == 'serial':
                child = router.connect_via_serial(reporter,
                                                  eol=self._eol,
                ...
                child = router.connect_via_telnet(reporter, eol=self._eol,
            ...
            default_file_system, software_ver, device_name, serial_num = router.get_device_info(
            ...
            router.enable_ssh(child, reporter, eol=self._eol,
                              label='ADVENTURES',
                              modulus=1024,
                              version=2,
                              commit=True)
            router.set_scp_source_interface(child, reporter, eol=self._eol,
                                            scp_interface_port=self._ethernet_port,
                                            commit=True)
            # For startup-config or running-config, use 'nvram' instead of default_file_system
            router.download_from_device_scp(child, reporter, eol=self._eol,
            ...
            router.upload_to_device_scp(child, reporter, eol=self._eol,
                                        device_file_system=default_file_system,
                                        remote_ip_addr=self._remote_ip_addr,
                                        remote_username=self._remote_username,
            ...
            router.set_tftp_source_interface(child, reporter, eol=self._eol,
                                             tftp_interface_port=self._ethernet_port,
                                             commit=True)
            # For startup-config or running-config, use 'nvram' instead of default_file_system
            router.download_from_device_tftp(child, reporter, eol=self._eol,
                                             device_file_system='nvram',
                                             remote_ip_addr=self._remote_ip_addr,
                                             file_to_download='startup-config',
                                             destination_filepath=self._file_to_transfer + '.tftp')
            router.upload_to_device_tftp(child, reporter, eol=self._eol,
                                         device_file_system=default_file_system,
                                         remote_ip_addr=self._remote_ip_addr,
            ...
            if connection_type == 'serial':
                router.close_serial_connection(child, reporter)
            else:
                router.close_telnet_connection(child, reporter)
